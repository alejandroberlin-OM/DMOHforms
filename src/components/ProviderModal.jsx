...
// KEY FIX: ProviderField is defined OUTSIDE ProviderModal so React doesn't
// unmount/remount it on every keystroke → focus maintained while typing

import { useState } from 'react';
import { C } from '../constants.js';
import { verifyPin, saveProvider } from '../utils/api.js';

// ── OUTSIDE the parent component — critical for focus stability ──────────────
function ProviderField({ label, field, ph, required, colSpan, form, setForm }) {
  return (
...
export default function ProviderModal({ provider, onSaved, onClose }) {
  ...
  const handleVerifyPin = async () => {
    ...
    try { await verifyPin(pin); setPinVerified(true); }
    catch { setPinError('Incorrect PIN. Please try again.'); }
    finally { setVerifying(false); }
  };

  const handleSave = async () => {
    setSaving(true); setSaveError('');
    try { const updated = await saveProvider(pin, form); onSaved(updated); }
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100, padding: 16 }}>
      <div style={{ background: C.white, borderRadius: 12, width: '100%', maxWidth: 560, maxHeight: '92vh', overflowY: 'auto', boxShadow: '0 20px 60px rgba(0,0,0,0.3)' }}>
        <div style={{ background: C.navy, padding: '14px 20px', borderRadius: '12px 12px 0 0', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <span style={{ color: C.white, fontWeight: 700, fontSize: 15 }}>{isNew ? 'Add Provider' : `Edit — Dr. ${provider.firstName} ${provider.lastName}`}</span>
          ...
          {!pinVerified ? (
            <div>
              ...
              <div style={{ display: 'flex', gap: 10 }}>
                <input type="password" value={pin} onChange={e => setPin(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleVerifyPin()} placeholder="Enter PIN"
            ...
            <div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                ...
                <ProviderField label="First Name" field="firstName" ph="Sarah" required {...fp} />
                <ProviderField label="Last Name" field="lastName" ph="Chen" required {...fp} />
                <ProviderField label="Specialty / Specialties" field="specialty" ph="Medical Oncology, Hematology" colSpan={2} {...fp} />
                <ProviderField label="CPSO No." field="cpso" ph="123456" required {...fp} />
                <ProviderField label="OHIP Billing No." field="billingNumber" ph="123456" {...fp} />
                <Divider label="Contact" />
                <ProviderField label="Telephone" field="tel" ph="416-555-0101" {...fp} />
                <ProviderField label="Fax" field="fax" ph="416-555-0102" {...fp} />
                <ProviderField label="Email" field="email" ph="dr@uhn.ca" colSpan={2} {...fp} />
                <Divider label="Hospital / Clinic" />
                <ProviderField label="Affiliated Hospital / Cancer Centre" field="hospital" ph="Princess Margaret Cancer Centre" colSpan={2} {...fp} />
                <ProviderField label="Clinic / Office Address" field="address" ph="610 University Ave" colSpan={2} {...fp} />
                <ProviderField label="City" field="city" ph="Toronto" {...fp} />
                <ProviderField label="Postal Code" field="postalCode" ph="M5G 2M9" {...fp} />
              </div>
